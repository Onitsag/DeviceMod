package fr.onitsag.faritech.api.print;
...
public class PrintingManager
{
    ...
    public static void registerPrint(ResourceLocation identifier, Class<? extends IPrint> classPrint)
    {
        try
        {
            ...
            if(FariTechMod.proxy.registerPrint(identifier, classPrint))
            {
                FariTechMod.getLogger().info("Registering print '" + classPrint.getName() + "'");
                registeredPrints.put(identifier.toString(), classPrint);
            }
            else
            {
                FariTechMod.getLogger().error("The print '" + classPrint.getName() + "' could not be registered due to a critical error!");
            }
        }
